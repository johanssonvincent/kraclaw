...
const (
	ChannelService_ListChannels_FullMethodName  = "/kraclaw.v1.ChannelService/ListChannels"
	ChannelService_SendMessage_FullMethodName   = "/kraclaw.v1.ChannelService/SendMessage"
	ChannelService_StreamInbound_FullMethodName = "/kraclaw.v1.ChannelService/StreamInbound"
)
...
type ChannelServiceClient interface {
	...
	StreamInbound(ctx context.Context, in *StreamInboundRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[InboundMessage], error)
}
...
func (c *channelServiceClient) StreamInbound(ctx context.Context, in *StreamInboundRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[InboundMessage], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ChannelService_ServiceDesc.Streams[0], ChannelService_StreamInbound_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[StreamInboundRequest, InboundMessage]{ClientStream: stream}
...
type ChannelService_StreamInboundClient = grpc.ServerStreamingClient[InboundMessage]

// ChannelServiceServer is the server API for ChannelService...
type ChannelServiceServer interface {
	ListChannels(context.Context, *ListChannelsRequest) (*ListChannelsResponse, error)
	SendMessage(context.Context, *SendMessageRequest) (*SendMessageResponse, error)
	StreamInbound(*StreamInboundRequest, grpc.ServerStreamingServer[InboundMessage]) error
...
func (UnimplementedChannelServiceServer) StreamInbound(*StreamInboundRequest, grpc.ServerStreamingServer[InboundMessage]) error {
	return status.Error(codes.Unimplemented, "method StreamInbound not implemented")
}
...
// UnsafeChannelServiceServer may be embedded to opt out of ...
...
// If the following call panics, it indicates UnimplementedC...
...
func _ChannelService_StreamInbound_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(StreamInboundRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(ChannelServiceServer).StreamInbound(m, &grpc.GenericServerStream[StreamInboundRequest, InboundMessage]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ChannelService_StreamInboundServer = grpc.ServerStreamingServer[InboundMessage]

// ChannelService_ServiceDesc is the grpc.ServiceDesc for Ch...
var ChannelService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "kraclaw.v1.ChannelService",
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "StreamInbound",
			Handler:       _ChannelService_StreamInbound_Handler,
			ServerStreams: true,
		},
	},
	Metadata: "kraclaw/v1/channel.proto",
}
